...
async function initializeDatabase() {
  try {
    ...
    // 테이블 동기화 (force: false로 변경하여 기존 데이터 보존)
    await sequelize.sync({ force: false });
    console.log('✅ 데이터베이스 테이블 동기화 완료');
    
    // 기존 샘플 데이터 확인
    const existingAdmin = await User.findOne({ where: { email: 'admin@moyak.com' } });
    const existingTestUser = await User.findOne({ where: { email: 'user@moyak.com' } });
    
    if (existingAdmin && existingTestUser) {
      console.log('✅ 기존 샘플 데이터가 이미 존재합니다. 초기화를 건너뜁니다.');
    ...
    if (!existingEvent) {
      ...
      console.log('✅ 기존 샘플 이벤트가 이미 존재합니다.');
    }
